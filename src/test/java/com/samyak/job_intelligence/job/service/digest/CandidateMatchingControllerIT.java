...
class CandidateMatchingControllerIT {
    ...
    @Test
    void shouldRefreshMatchingAndReturnDailyDigest() throws Exception {
        ...
        when(digestService.generate(candidateProfileId, limit))
                .thenReturn(digest);

        mockMvc.perform(
                        post(
                                "/api/v1/candidates/{candidateProfileId}/matching/refresh",
                                candidateProfileId
                        )
        ...
        verify(digestService)
                .generate(candidateProfileId, limit);
    }
